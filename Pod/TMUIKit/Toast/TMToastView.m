...
- (instancetype)initWithToast:(TMToast *)toast removeCompletion:(TMToastViewDidRemove)removeCompletion
{
    ...
    if (self) {
        ...
        UILabel *label = UILabel.tm_label.tm_text(toast.message).tm_textColor([UIColor whiteColor]).tm_lineBreakMode(NSLineBreakByWordWrapping).tm_numberOfLines(0).tm_textAlignment(toast.textAlignment).tm_preferredMaxLayoutWidth(300).tm_superView(self);
